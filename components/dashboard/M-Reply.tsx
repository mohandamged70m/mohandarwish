...
export default function MReply({ email, isDark, onClose, onSent, notify }: MReplyProps) {
    ...
    const send = async () => {
        ...
        try {
            ...
            if (files.length) {
                const { getStorage, ref, uploadBytes, getDownloadURL } = await import('@/lib/dash-storage');
            ...
            const { httpsCallable, getFunctions } = await import('@/lib/dash-functions');
        ...
        >
            ...
            >
                ...
                <div className="flex items-center justify-between gap-3 p-4 border-t border-[var(--section-border)]">
                    <span className="text-xs text-sec truncate">Sent via your Resend sender address</span>
