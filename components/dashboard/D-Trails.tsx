...
import {
    ...
    MousePointer2, Eye, Globe, ChevronLeft, ChevronRight, Trophy, ExternalLink,
...
const CustomTooltip = ({ active, payload, isDark }: TooltipProps<number, string> & { isDark?: boolean; payload?: { payload: ChartDataPoint }[] }) => {
        ...
        const linksUnsub = onSnapshot(collection(db, 'Analytics', 'Links', 'Items'), snap => {
            const rows: LinkRow[] = snap.docs.map(d => {
                ...
                return {
                    ...
                    url: `${window.location.origin}/mohanddarwish/${data.Code || ''}`,
                };
            });
