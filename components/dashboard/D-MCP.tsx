...
// The MCP server's URL - shown by default so the field is never empty.
const DEFAULT_URL = 'https://mcp.mohanddarwish.vercel.app';
// Legacy vanity domain, retired - any saved config still pointing here falls
// back to DEFAULT_URL so the panel never shows a dead link.
const LEGACY_URLS = ['https://mcp.temrevil.com'];
...
const DMcpPanel = ({ isDark }: { isDark: boolean }) => {
    ...
    const effectiveUrl = cfg.url && !LEGACY_URLS.includes(cfg.url.trim()) ? cfg.url : DEFAULT_URL;
