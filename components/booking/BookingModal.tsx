...
export function BookingModal({ open, onClose, initialTab = "meeting", hideTabs = false }: Props) {
  ...
  useEffect(() => {
    ...
    const h = (e: KeyboardEvent) => {
      if (e.key === "Escape" && !(document.querySelector("[role='dialog'][aria-label='Pick a custom time']"))) onClose();
    };
    document.addEventListener("keydown", h);
    prevOverflowRef.current = document.body.style.overflow;
    document.body.style.overflow = "hidden";
    // Lenis drives scroll at window level and ignores body overflow — stop it
    // so wheel/touch over the modal scrolls the modal, not the page behind.
                ...
                {activeTab === "meeting" ? (
                  <motion.div key="meeting" custom={tabDirection} variants={tabVariants} initial="enter" animate="center" exit="exit" transition={{ duration: 0.3, ease: [0.4, 0, 0.2, 1] }} style={{ flex: isMobile ? "0 0 auto" : 1, minHeight: 0, display: isMobile ? "flex" : "grid", flexDirection: isMobile ? "column" : "row", gridTemplateColumns: isMobile ? "none" : "1.2fr 1fr", gap: isMobile ? "40px" : "32px", overflowY: isMobile ? "visible" : "hidden", padding: isMobile ? "0 16px 24px" : "0 24px 12px", height: isMobile ? "auto" : "100%" } as unknown as React.CSSProperties}>
                          ...
                          <AnimatePresence mode="popLayout" initial={false} custom={direction}>
                            <motion.div key={calendarDate.toISOString()} role="grid" aria-label={calendarDate.toLocaleDateString("en-US", { month: "long", year: "numeric" })} custom={direction} variants={{ enter: (d: number) => ({ x: d > 0 ? "100%" : "-100%", opacity: 0 }), center: { x: 0, opacity: 1 }, exit: (d: number) => ({ x: d > 0 ? "-100%" : "100%", opacity: 0 }) }} initial="enter" animate="center" exit="exit" transition={{ type: "spring", stiffness: 300, damping: 30 }} style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", gap: "8px", textAlign: "center" }}>
                              ...
                              {Array.from({ length: getDaysInMonth(calendarDate).days }).map((_, i) => {
                                ...
                                return (<div key={day} role="button" tabIndex={isBookable ? 0 : -1} aria-disabled={!isBookable} aria-pressed={isSelected} aria-selected={isSelected} aria-label={`${date.toLocaleDateString("en-US", { weekday: "long", month: "long", day: "numeric", year: "numeric" })}${isBookable ? "" : ", unavailable"}`} onClick={() => { if (isBookable) setSelectedDate(date); }} onKeyDown={(e) => { if (isBookable && (e.key === "Enter" || e.key === " ")) { e.preventDefault(); setSelectedDate(date); } }} style={{ width: "100%", height: "clamp(38px, 6vh, 52px)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", borderRadius: "14px", cursor: isBookable ? "pointer" : "default", position: "relative", opacity: isBookable ? 1 : 0.4, outlineOffset: "2px" }}>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
                      <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
                        <div><label htmlFor="booking-name" className="label-help">Name *<HintTooltip text="Warning: your name will show in the calendar, if you want to hide it, please use a nickname." isDark={isDark} /></label><input id="booking-name" aria-label="Name" autoComplete="name" required className="dashboard-input" style={{ borderRadius: "12px", width: "100%" }} placeholder="Your Name" value={meetingData.name} onChange={(e) => setMeetingData({ ...meetingData, name: e.target.value })} /></div>
                        <div><label htmlFor="booking-email" className="label-help">Email *<HintTooltip text="Please use a correct email address. I will send the Google Calendar invitation and meeting link directly to this inbox." isDark={isDark} /></label><input id="booking-email" type="email" aria-label="Email" autoComplete="email" required className="dashboard-input" style={{ borderRadius: "12px", width: "100%" }} placeholder="Your Email" value={meetingData.email} onChange={(e) => setMeetingData({ ...meetingData, email: e.target.value })} /></div>
                      </div>
                  ...
                  <motion.div key="message" custom={tabDirection} variants={tabVariants} initial="enter" animate="center" exit="exit" transition={{ duration: 0.3, ease: [0.4, 0, 0.2, 1] }} style={{ flex: isMobile ? "0 0 auto" : 1, minHeight: 0, display: "flex", flexDirection: isMobile ? "column" : "row", justifyContent: isMobile ? "flex-start" : "center", overflowY: isMobile ? "visible" : "hidden", padding: isMobile ? "0 16px 24px" : "0 24px 12px", height: isMobile ? "auto" : "100%" }}>
                    <form onSubmit={handleSubmit} className={!isMobile ? "glass-panel-deep" : ""} style={{ width: "100%", maxWidth: isMobile ? "none" : "640px", height: isMobile ? "auto" : "100%", display: "flex", flexDirection: "column", overflow: "hidden", borderRadius: isMobile ? "0" : "24px", boxShadow: isMobile ? "none" : "0 18px 44px -22px rgba(0,0,0,0.16), 0 44px 96px -40px rgba(0,0,0,0.2)", willChange: "transform", position: "relative" }}>
                      ...
                      <div data-lenis-prevent onScroll={(e) => setMessageScrolled(e.currentTarget.scrollTop > 4)} className={!isMobile ? "hide-scrollbar" : ""} style={{ flex: 1, minHeight: 0, overflowY: isMobile ? "visible" : "auto", padding: isMobile ? "0" : "4px 24px 24px", display: "flex", flexDirection: "column", gap: "20px" }}>
                        <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
                          <div><label htmlFor="msg-name" className="input-label font-semibold">Name *</label><div className="input-container"><User size={18} className="input-icon" aria-hidden="true" /><input id="msg-name" name="name" aria-label="Name" autoComplete="name" value={formData.name} onChange={handleInputChange} required className="input-with-icon" placeholder="Full Name" /></div></div>
                          <div><label htmlFor="msg-email" className="input-label font-semibold">Email *</label><div className="input-container"><Mail size={18} className="input-icon" aria-hidden="true" /><input id="msg-email" type="email" name="email" aria-label="Email" autoComplete="email" value={formData.email} onChange={handleInputChange} required className="input-with-icon" placeholder="name@example.com" /></div></div>
                        </div>
                        <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
                          <div><label htmlFor="msg-phone" className="input-label font-semibold">Phone Number</label><div className="input-container"><Phone size={18} className="input-icon" aria-hidden="true" /><input id="msg-phone" type="tel" name="number" aria-label="Phone number" autoComplete="tel" value={formData.number} onChange={handleInputChange} className="input-with-icon" placeholder="+1 (555) 123-4567" /></div></div>
